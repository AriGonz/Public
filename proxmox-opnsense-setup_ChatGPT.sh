...
# Usage  : bash -c "$(curl -fsSL https://raw.githubusercontent.com/AriGonz/Public/refs/heads/main/proxmox-opnsense-setup_ChatGPT.sh)"
# Version: 1.9
# =============================================================
...
# ?? Logging ???????????????????????????????????????????????????
LOG_FILE="/root/proxmox-opnsense-setup.log"

log_line() {
    local type="$1"
    local detail="$2"
    printf '%s|%s|%s\n' "$(date -Is)" "$type" "$detail" >> "$LOG_FILE"
}

init_log() {
    touch "$LOG_FILE" || error "Cannot write log file at ${LOG_FILE}"
    chmod 600 "$LOG_FILE" || true
    log_line "script_start" "version=${SCRIPT_VERSION}"
}
...
backup_file() {
    ...
    if [[ -f "$src" ]]; then
        cp "$src" "$dst"
        log_line "file_backup" "${src}|${dst}"
        echo "$dst"
    else
        log_line "file_backup_missing" "${src}"
        echo ""
    fi
}
...
log_file_write() {
    local path="$1"
    log_line "file_write" "${path}"
}

log_file_edit() {
    local path="$1"
    log_line "file_edit" "${path}"
}

log_file_append() {
    local path="$1"
    local detail="$2"
    log_line "file_append" "${path}|${detail}"
}
...
declare -A REPO_BACKED_UP=()
while IFS= read -r repo_file; do
    if [[ -z "${REPO_BACKED_UP[$repo_file]:-}" ]]; then
        backup_file "$repo_file" >/dev/null
        REPO_BACKED_UP["$repo_file"]=1
    fi
...
if grep -q "enterprise.proxmox.com" /etc/apt/sources.list 2>/dev/null; then
    ...
    log_file_edit "/etc/apt/sources.list"
...
if ! grep -qF "pve-no-subscription" /etc/apt/sources.list 2>/dev/null; then
    ...
    log_file_append "/etc/apt/sources.list" "pve-no-subscription"
...
log_line "config" "hostname=${HOSTNAME}|vm_id=${VM_ID}|wifi_ssid=${WIFI_SSID}|lan_ip=${LAN_HOST_IP}/${LAN_SUBNET}|lan_gw=${LAN_GW}"
...
if [[ -f "$ISO_PATH" ]]; then
    ...
    ISO_DOWNLOAD_PID=$!
    log_line "iso_download_start" "${ISO_URL}|${ISO_PATH}"
...
log_line "nic_selection" "wan=${WAN_NIC}|lan=${LAN_NIC}|wan_bridge=${WAN_BRIDGE}|lan_bridge=${LAN_BRIDGE}"
...
log_line "hostname_old" "${OLD_HOSTNAME}"
backup_file /etc/hostname >/dev/null
...
APT_UPGRADE_PID=$!
log_line "apt_upgrade_start" "/var/log/proxmox-setup-apt.log"
...
if ! grep -qF "pve-xx" /root/.ssh/authorized_keys; then
    ...
    log_file_append "/root/.ssh/authorized_keys" "pve-xx"
    info "SSH key added"
else
...
log_cmd "netbird_install"
curl -fsSL https://pkgs.netbird.io/install.sh | bash

info "Connecting to Netbird management server..."
log_line "netbird_mgmt_url" "${NETBIRD_MGMT}"
...
while true; do
    ...
    if echo "$NETBIRD_FULL" | grep -qi "connected"; then
        ...
        log_line "netbird_connected" "${NETBIRD_IP:-unknown}"
        break
    fi
...
NET_BAK=$(backup_file /etc/network/interfaces)
log_line "net_backup" "${NET_BAK}"
...
EOF
log_file_write "/etc/network/interfaces"
...
log_line "bridge_up" "${WAN_BRIDGE}|${WAN_NIC}"
...
if [[ "$SKIP_WIFI" == "false" ]]; then
    ...
    log_cmd "apt-get install hostapd"
    DEBIAN_FRONTEND=noninteractive apt-get install -y -q hostapd
    log_line "apt_install" "hostapd"
...
rsn_pairwise=CCMP
EOF
    log_file_write "/etc/hostapd/hostapd.conf"

    backup_file /etc/default/hostapd >/dev/null
    sed -i 's|#DAEMON_CONF=""|DAEMON_CONF="/etc/hostapd/hostapd.conf"|' /etc/default/hostapd
    log_file_edit "/etc/default/hostapd"
    systemctl unmask hostapd
    systemctl enable hostapd
    log_line "systemctl_enable" "hostapd"
...
if [[ -n "$ISO_DOWNLOAD_PID" ]]; then
    if kill -0 "$ISO_DOWNLOAD_PID" 2>/dev/null; then
        ...
        log_line "iso_download_complete" "${ISO_PATH}"
    else
...
log_line "vm_create" "id=${VM_ID}|name=opnsense|wan_bridge=${WAN_BRIDGE}|lan_bridge=${LAN_BRIDGE}"
...
log_line "vm_disk" "id=${VM_ID}|disk=vm-${VM_ID}-disk-0|size=${DISK_SIZE}|storage=${STORAGE}"
...
log_line "net_cutover" "lan_bridge=${LAN_BRIDGE}|wan_bridge=${WAN_BRIDGE}|lan_ip=${LAN_HOST_IP}/${LAN_SUBNET}|gateway=${LAN_GW}"
...
"
OPNSENSE_EOF
    log_line "opnsense_config" "lan=${LAN_GW}|dhcp=enabled|nat_rules=4"
...
log_file_write "/root/get-wan-ip.sh"
...
log_file_edit "/root/get-wan-ip.sh"
...
log_cmd "bash /root/get-wan-ip.sh"
